...
class AppContainer(QWidget):
    def __init__(self,user_email=None):
        ...
        pages = {
            ...
            "Live From Steets Aid": self.computer_vision
        }
        ...
        layout.addWidget(nav, stretch=0)
...
class FullMapWindow(QWidget):
    def __init__(self):
        ...
        full_map_layout.addWidget(self.exit_button)
...
class StartWindow(QWidget):
    ...
    def handle_search(self):
        ...
        try:
            df = pd.read_csv(resource_path('csv_support/County.csv'))
...
class FireRiskCircle(QFrame):
    ...
    def paintEvent(self, event):
        ...
        # Progress arc (red) - clockwise from top (90 degrees)
        painter.setPen(QPen(Qt.red, 12))
        start_angle = 90 * 16  # 90 degrees, 1/16 degree units
        span_angle = int(-self._current_risk / 100 * 360 * 16)  # negative fill
...
class VisualizeWindow(QWidget):
    ...
    def update_imported_model_mode(self):
        ...
        else:  # edit mode - semi-transparent + edges visible
...
class Bridge(QObject):
    ...
    def openExternal(self, url):
        webbrowser.open(url)
        #t = threading.Thread(target=test.main, daemon=True)
        #t.start()
        subprocess.Popen([sys.executable, "-u", "test.py"])

class CV_Section(QWidget):
    def __init__(self):
        super().__init__()
        self.init_ui()
    ...
    def init_ui(self):
        ...
        self.ai_map_layout=QVBoxLayout()
        ...
        if getattr(sys, 'frozen', False):
            url="https://firehouseromania.com/firmsimportedmap"
        else:
            url="http://127.0.0.1:5000/firmsimportedmap"
        self.browser.setUrl(QUrl(url))
        self.ai_map_layout.addWidget(self.browser)

        self.setLayout(self.ai_map_layout)
    ...
    def load_user_data(self):
        try:
            ...
            if response.status_code == 200:
                return response.json()  # Return {"name": ..., "password": ...}
            else:
                return {}
        except Exception as e:
